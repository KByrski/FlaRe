...
static __global__ void __launch_bounds__(threads_per_block, 1) Forward_CUDA_kernel(
	half2 *conditioning_variable_ptr,
	float *features_P_hit_object_ptr,
	float3 *RGB_ptr,
	...
	while (true) {
		...
		for (int i = 0; i < 16; ++i) {
			...
			float opacity1 = __ldg(A + ind_clamped);
			asm volatile ("rcp.approx.ftz.f32 %0, %1;" : "=f"(opacity1) : "f"(1.0f + __expf(-opacity1)));
			...
			if ((ind != -1) && (T >= T_threshold)) {
				const float3 RGB_param = RGB_ptr[ind_clamped];
				RGBA.x *= RGB_param.x;
				RGBA.y *= RGB_param.y;
				RGBA.z *= RGB_param.z;
				if (is_shadowed) {
					RGBA.x *= shadow_multiplier;
					RGBA.y *= shadow_multiplier;
					RGBA.z *= shadow_multiplier;
				}

				// !!! !!! !!!
				RGB_aggregated.x = __fmaf_rn(RGBA.x, opacity, RGB_aggregated.x);
				RGB_aggregated.y = __fmaf_rn(RGBA.y, opacity, RGB_aggregated.y);
				RGB_aggregated.z = __fmaf_rn(RGBA.z, opacity, RGB_aggregated.z);
				// !!! !!! !!!
		...
		if (tid < number_of_rays) {
			...
			t_min_ptr[pixel_ind] = (ind != -1) ? nextafterf(t_hit, INFINITY) : INFINITY; // !!! !!! !!!
			T_ptr[pixel_ind] = T; // !!! !!! !!!
...
void CPyOptiXFLAREVIEWERRenderer_CUDA::Forward(
	...
	error_CUDA = cudaFuncSetAttribute(Forward_CUDA_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, size);
	...
	int *ray_indices_initial_ptr = ray_indices_initial;
	float3 *O_ptr = O;
	float3 *v_ptr = v;
	float *T_ptr = T;
	float *t_min_ptr = t_min;
	int *is_active_ptr = is_active;

	while (true) {
		// Visualization
		error_CUDA = cudaMemcpy(ray_indices_initial_ptr, ray_indices[pass_num & 1], sizeof(int) * number_of_rays, cudaMemcpyDeviceToDevice);
		if (error_CUDA != CUDA_SUCCESS) throw 0;

		// !!! !!! !!!
		int number_of_rays_initial = number_of_rays;
		// !!! !!! !!!
		...
		launchParams.O = (float3 *)O_ptr;
		launchParams.v = (float3 *)v_ptr;
		launchParams.t_min = t_min_ptr;
		launchParams.AS = IAS;
		launchParams.t = t_hit;
		launchParams.indices = indices_hit;

		launchParams.number_of_Gaussians = number_of_Gaussians;
		launchParams.is_mesh = is_mesh;
		launchParams.buffer1 = buffer1;
		launchParams.buffer2 = buffer2;

		launchParams.lights = lights;
		launchParams.number_of_lights = number_of_lights;

		launchParams.epsilon = epsilon;
		launchParams.Ia = Ia;
		...
		while (number_of_rays > 0) {
			...
			Forward_CUDA_kernel<<<min((number_of_rays + (threads_per_block - 1)) / threads_per_block, SM_count), threads_per_block, size, stream>>>(
				...
				t_hit, indices_hit, t_min_ptr, T_ptr, is_active_ptr, bitmap,
			...
			try {
				thrust::exclusive_scan(
					thrust::cuda::par.on(stream),
					thrust::device_pointer_cast(is_active_ptr),
					thrust::device_pointer_cast(is_active_ptr) + number_of_rays,
		...
		launchParams.bitmap = bitmap;
		launchParams.ray_indices_initial = ray_indices_initial_ptr;
		launchParams.is_mesh = is_mesh;
		launchParams.buffer1 = buffer1;
		launchParams.buffer2 = buffer2;
		launchParams.materials = materials;
		launchParams.M4N = M4N;
		launchParams.T_ptr = T_ptr;
		launchParams.T_threshold = T_threshold;
		launchParams.bg_color = bg_color;
		launchParams.is_active = is_active_ptr;
		launchParams.entry = stack[stack_size];
		...
		try {
			stack[stack_size].number_of_rays = thrust::reduce(
				thrust::cuda::par.on(stream),
				thrust::device_pointer_cast(stack[stack_size].is_active),
				thrust::device_pointer_cast(stack[stack_size].is_active) + number_of_rays_initial
			);
			// !!! !!! !!!
		} catch (...) {
			throw 0;
		...
		if (stack[stack_size].number_of_rays > 0) {
			if (stack_size < max_recursion_depth) {
				error_CUDA = cudaMemcpy(stack[stack_size].ray_indices_initial, ray_indices_initial_ptr, sizeof(int) * number_of_rays_initial, cudaMemcpyDeviceToDevice);
				if (error_CUDA != cudaSuccess) throw 0;

				// !!! !!! !!!
				stack[stack_size].number_of_rays_initial = number_of_rays_initial;
				stack[stack_size].processed = false;
				++stack_size;
				// !!! !!! !!!
				...
				try {
					...
					number_of_rays = thrust::reduce(
						thrust::cuda::par.on(stream),
						thrust::device_pointer_cast(is_active_ptr),
						thrust::device_pointer_cast(is_active_ptr) + number_of_rays_initial
					);
					// !!! !!! !!!
				} catch (...) {
					throw 0;
				...
				do {
					--stack_size;
				} while ((stack_size >= 0) && stack[stack_size].processed);

				// !!! !!! !!!
				if (stack_size < 0) break;
				// !!! !!! !!!

				ray_indices_initial_ptr = stack[stack_size].ray_indices_initial;
				O_ptr = stack[stack_size].O;
				v_ptr = stack[stack_size].v;
				T_ptr = stack[stack_size].T;
				t_min_ptr = stack[stack_size].t_min;
				is_active_ptr = stack[stack_size].is_active;

				// !!! !!! !!!
				number_of_rays_initial = stack[stack_size].number_of_rays_initial;
				number_of_rays = stack[stack_size].number_of_rays;
				stack[stack_size].processed = true;
				++stack_size;
				// !!! !!! !!!
			}
		} else {
		...
		try {
			...
			thrust::scatter_if(
				thrust::cuda::par.on(stream),
				thrust::device_pointer_cast(ray_indices_initial_ptr),
				thrust::device_pointer_cast(ray_indices_initial_ptr) + number_of_rays_initial,
				thrust::device_pointer_cast(indices_hit),
				thrust::device_pointer_cast(is_active_ptr),
				thrust::device_pointer_cast(ray_indices[pass_num & 1])
			);
