...
// Paper: four collision-free 2D LUT levels, two features each, plus a 96D
// per-primitive descriptor and positional encoding of the view direction.
...
> static __device__ __forceinline__ void HashEncoding2D_CUDA_kernel(
	...
	// Paper: "LUT-Encoding". Direct row-major vertex lookup avoids hash
	// collisions while bilinear interpolation preserves continuity in (u, v).
...
// Paper: assemble Phi's 128D input: LUT(u, v), gamma(d), and z_i.
...
static __device__ __forceinline__ void Forward(
	...
	// Paper: the shared auto-decoder has two 64-wide Hardswish hidden layers.
	// Hardswish
	#pragma unroll
	for (int i = 0; i < 8; i += 2) {
...
static __global__ void __launch_bounds__(threads_per_block, 1) Forward_inference_CUDA_kernel(
	half2 *conditioning_variable_ptr,
	float *features_P_hit_object_ptr,
	...
	while (true) {
		...
		for (int i = 0; i < 16; ++i) {
			...
			// Paper: the four decoder outputs use ReLU for RGB and sigmoid for opacity.
			// 3 x ReLU + Sigmoid
			...
			// Paper: alpha_i = alpha_const * alpha_MLP * N(u, v; kappa),
			// multiplied by current transmittance T for front-to-back compositing.
			float opacity = T * (opacity1 * __expf(-0.5f * kappa_inv * r_squared_pow_kappa) * RGBA.w); // !!! !!! !!!

			if ((ind != -1) && (T >= T_threshold)) {
				// !!! !!! !!!
				// Feature visualization replaces only the final per-primitive RGB.
				// Opacity and compositing remain identical to standard FlaRe inference.
				float3 hit_RGB;
				if (override_colors != nullptr) {
					hit_RGB = override_colors[ind_clamped];
				} else {
					hit_RGB = make_float3(
						RGBA_param.x * RGBA.x,
						RGBA_param.y * RGBA.y,
				...
				// Paper: standard FlaRe compositing accumulates c_const * c_MLP.
				// Feature visualization may override that final per-primitive color.
				RGB_aggregated.x = __fmaf_rn(hit_RGB.x, opacity, RGB_aggregated.x);
				RGB_aggregated.y = __fmaf_rn(hit_RGB.y, opacity, RGB_aggregated.y);
				RGB_aggregated.z = __fmaf_rn(hit_RGB.z, opacity, RGB_aggregated.z);
...
void CPyOptiXFLARERenderer_CUDA::Forward_inference(
	...
	error_CUDA = cudaFuncSetAttribute(Forward_inference_CUDA_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, size);
	...
	while (number_of_rays > 0) {
		...
		Forward_inference_CUDA_kernel<<<min((number_of_rays + (threads_per_block - 1)) / threads_per_block, SM_count), threads_per_block, size, stream>>>(
