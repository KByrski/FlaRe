...
static __global__ void __launch_bounds__(threads_per_block, 1) GetMedianDepth_base_CUDA_kernel(
	float3 *O_ptr,
	float3 *v_ptr,
	int *ray_indices,
	float *t_hit_ptr,
...
void CPyOptiXFLARERenderer_CUDA::GetMedianDepth_base(
	float3 *O,
	float3 *v,
	...
	while (number_of_rays > 0) {
		...
		GetMedianDepth_base_CUDA_kernel<<<min((number_of_rays + (threads_per_block - 1)) / threads_per_block, SM_count), threads_per_block, 0, stream>>>(
			O, v,
