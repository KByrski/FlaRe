...
static __global__ void __launch_bounds__(threads_per_block, 1) GetMedianDepth_CUDA_kernel(
	half2 *conditioning_variable_ptr,
	float *features_P_hit_object_ptr,
	...
	while (true) {
		...
		float2 depth_and_index = make_float2(INFINITY, __int_as_float(-1)); // !!! !!! !!!
		...
		for (int i = 0; i < 16; ++i) {
			...
			if ((ind != -1) && (T >= T_threshold) && (T > 0.5)) { // !!! !! !!!
				depth_and_index = make_float2(t_hit, __int_as_float(ind));
				T = T - opacity;
			}
		}
		...
		if (tid < number_of_rays) {
			bool is_active = ((ind != -1) && (T >= T_threshold) && (T > 0.5));

			t_min_ptr[pixel_ind] = (ind != -1) ? nextafter(t_hit, INFINITY) : INFINITY; // !!! !!! !!!
			T_ptr[pixel_ind] = T; // !!! !!! !!!

			if ((pass_num == 0) || isfinite(depth_and_index.x))
				depth_and_index_ptr[pixel_ind] = depth_and_index;
...
void CPyOptiXFLARERenderer_CUDA::GetMedianDepth(
	...
	error_CUDA = cudaFuncSetAttribute(GetMedianDepth_CUDA_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, size);
	...
	while (number_of_rays > 0) {
		...
		GetMedianDepth_CUDA_kernel<<<min((number_of_rays + (threads_per_block - 1)) / threads_per_block, SM_count), threads_per_block, size, stream>>>(
