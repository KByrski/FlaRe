...
class GaussianModel:
    ...
    def __init__(self): #, sh_degree : int):
        ...
        # added
        self.w1_uv = torch.empty(64, 8, dtype=torch.float32, device="cuda")
        self.w1_v = torch.empty(64, 24, dtype=torch.float32, device="cuda")
        self.w1_conditioning = torch.empty(64, 96, dtype=torch.float32, device="cuda")
        self.b1 = torch.zeros(64, dtype=torch.float32, device="cuda")
        self.w2 = torch.empty(64, 64, dtype=torch.float32, device="cuda")
        self.b2 = torch.zeros(64, dtype=torch.float32, device="cuda")
        self.w3 = torch.empty(16, 64, dtype=torch.float32, device="cuda")
        self.b3 = torch.zeros(16, dtype=torch.float32, device="cuda")
        # Paper: "LUT-Encoding". Four 2D grids with two features per vertex:
        # 2 * (16^2 + 25^2 + 40^2 + 64^2) = 13,154 trainable values.
        self.features = torch.empty(13154, dtype=torch.float32, device="cuda")
        # ### ### ### ### ###
    ...
    def create_from_pcd(self, pp, pcd : BasicPointCloud, spatial_lr_scale : float): # added
        ...
        self.conditioning_variable = torch.empty(self.number_of_Gaussians, 96, dtype=torch.float32, device="cuda")
        ...
        self.RGB = torch.tensor(np.asarray(pcd.colors)).float().cuda(); # !!! !!! !!!
        self.A = torch.logit(torch.full((self.number_of_Gaussians, 1), pp.initial_opacity, dtype=torch.float32, device="cuda"))
        self.k = torch.full((self.number_of_Gaussians, 1), pp.initial_k, dtype=torch.float32, device="cuda")
        # Store raw kappa so the renderer can enforce kappa = 1 + softplus(k).
        self.k = (self.k - 1.0) + torch.log(-torch.expm1(-self.k + 1.0)); # inverse softplus
        
        self.m = torch.tensor(np.asarray(pcd.points)).float().cuda()
        dist2 = torch.clamp_min(distCUDA2(torch.from_numpy(np.asarray(pcd.points)).float().cuda()), 0.0000001)
        self.s = torch.log(torch.sqrt(dist2))[...,None].repeat(1, 2)
        self.q = torch.randn(self.number_of_Gaussians, 4, dtype=torch.float32, device="cuda")
        self.q = torch.nn.functional.normalize(self.q, dim=1)
    # ### ### ### ### ###
    ...
    def training_setup(self, training_args):
        ...
        param_group = [
            {'params': [self.RGB], 'lr': lr_RGB_current},                            # 0
            {'params': [self.A], 'lr': lr_A_current},                                # 1
            {'params': [self.k], 'lr': lr_k_current},                                # 2
            {'params': [self.w1_uv], 'lr': lr_w1_uv_current},                        # 3
            {'params': [self.w1_v], 'lr': lr_w1_v_current},                          # 4
            {'params': [self.w1_conditioning], 'lr': lr_w1_conditioning_current},    # 5
            {'params': [self.b1], 'lr': lr_b1_current},                              # 6
            {'params': [self.w2], 'lr': lr_w2_current},                              # 7
            {'params': [self.b2], 'lr': lr_b2_current},                              # 8
            {'params': [self.w3], 'lr': lr_w3_current},                              # 9
            {'params': [self.b3], 'lr': lr_b3_current},                              # 10
            {'params': [self.conditioning_variable], 'lr': lr_conditioning_current}, # 11
            {'params': [self.features], 'lr': lr_features_current},                  # 12
            {'params': [self.m], 'lr': lr_m_current},                                # 13
            {'params': [self.s], 'lr': lr_s_current},                                # 14
            {'params': [self.q], 'lr': lr_q_current},                                # 15
        ]
        self.optimizer = torch.optim.Adam(param_group)
    # ### ### ### ### ###
    ...
    def load_checkpoint(self, lp, path):
        ...
        self.RGB = checkpoint[0]
        self.A = checkpoint[1]
        self.k = checkpoint[2]
        self.w1_uv = checkpoint[3]
        self.w1_v = checkpoint[4]
        self.w1_conditioning = checkpoint[5]
        self.b1 = checkpoint[6]
        self.w2 = checkpoint[7]
        self.b2 = checkpoint[8]
        self.w3 = checkpoint[9]
        self.b3 = checkpoint[10]
        self.conditioning_variable = checkpoint[11]
        self.features = checkpoint[12]
        self.m = checkpoint[13]
        self.s = checkpoint[14]
        self.q = checkpoint[15]
        # ##########################################################################################
        
        self.iteration = int(checkpoint[16]['state'][0]['step'])
        self.training_setup(lp)
        # ##########################################################################################
        
        self.optimizer.load_state_dict(checkpoint[16])
        self.training_time = checkpoint[17]